...
import { Area, Check, DungeonName, ExitMapping, RegularDungeon, isDungeon } from '../newApp/DerivedState';
import { Logic, LogicalCheck, makeDay, makeNight } from '../logic/Logic';
import { cubeCheckToGoddessChestCheck, requiredDungeonsCompletedFakeRequirement, sothItemReplacement, sothItems, triforceItemReplacement, triforceItems } from '../newApp/TrackerModifications';
...
export const rawItemCountsSelector = (state: RootState) => state.tracker.inventory;
...
function getNumLooseGratitudeCrystals(logic: Logic, checkedChecks: TrackerState['checkedChecks']) {
    return checkedChecks.filter((check) => logic.checks[check].type === 'loose_crystal').length;
}
...
export const totalGratitudeCrystalsSelector = createSelector(
    ...
    (logic, checkedChecks, packCount) => {
        const looseCrystalCount = getNumLooseGratitudeCrystals(logic, checkedChecks);
...
export const itemCountsSelector = createSelector(
    ...
    (logic, rawItemCounts, checkedChecks) => {
        ...
        result['Gratitude Crystal'] = getNumLooseGratitudeCrystals(logic, checkedChecks);
        return result;
    }
);
...
export const allowedStartingEntrancesSelector = createSelector(
    ...
    (logic, randomizeStart) => {
        ...
        return Object.entries(logic.areaGraph.entrances).filter(([, def]) => {
...
export const settingsImplicationsSelector = createSelector(
    [logicSelector, optionsSelector, settingsSelector, activeVanillaConnectionsSelector, mappedExitsSelector],
    mapSettings,
);

function mapSettings(logic: Logic, options: OptionDefs, settings: TypedOptions, activeVanillaConnections: Record<string, string>, mappedExits: Record<string, string | undefined>) {
    ...
    const raiseGotExpr = settings['got-start'] ? LogicalExpression.true(logic.numItems) : new LogicalExpression([vec(impaSongCheck)]);
    ...
    let hordeDoorExpr = settings['triforce-required'] ? new LogicalExpression([vec(completeTriforceReq)]) : LogicalExpression.true(logic.numItems);

    const dungeonsExpr = new LogicalExpression([logic.items[requiredDungeonsCompletedFakeRequirement][0]]);
...
function mapChecks(logic: Logic, requiredDungeons: string[], checkedChecks: string[]) {
    const implications: { [bitIndex: number]: LogicalExpression } = {};
    ...
    implications[logic.items[requiredDungeonsCompletedFakeRequirement][1]] = requiredDungeonsCompleted ? LogicalExpression.true(logic.numItems) : LogicalExpression.false();

    return implications;
}

export const inLogicBitsSelector = createSelector(
    [logicSelector, settingsImplicationsSelector, inventoryImplicationsSelector, checkImplicationsSelector],
    (logic, settingsImplications, inventoryImplications, checkImplications) => interpretLogic(logic, settingsImplications, inventoryImplications, checkImplications),
);

export const dungeonCompletedSelector = currySelector(createSelectorWeakMap(
    [(_state: RootState, name: DungeonName) => name, logicSelector, inLogicBitsSelector],
    (name, logic, inLogicBits) => name !== 'Sky Keep' && inLogicBits.test(logic.items[dungeonCompletionRequirements[name]][1])
))

export const checkSelector = currySelector(createSelectorWeakMap(
    [(_state: RootState, checkId: string) => checkId, logicSelector, inLogicBitsSelector, checkedChecksSelector],
    (checkId, logic, inLogicBits, checkedChecks): Check => {
        const inLogic = inLogicBits.test(logic.items[checkId][1]);
        ...
        return {
            ...
            logicalState: inLogic ? 'inLogic' : 'outLogic',
        }
    }
));
...
export const isCheckBannedSelector = createSelector(
    ...
    (logic, bannedLocations, rupeeSanity, shopSanity, tadtoneSanity, silentRealmTreasuresanity, silentRealmTreasureAmount) => {
...
export const areasSelector = createSelector(
    ...
    (logic, checkedChecks, isCheckBanned, inLogicBits, isAreaNonprogress, rawCheckOrder): Area[] => {
        const areasList = Object.entries(logic.checksByArea).map(([area, checks]) => {
            ...
            const [extraChecks, regularChecks] = _.partition(
                progressChecks,
                (check) =>
                    logic.checks[check].type === 'tr_cube' || logic.checks[check].type === 'loose_crystal',
            );
    
            const remaining = regularChecks.filter((check) => !checkedChecks.includes(check));
            const inLogic = remaining.filter((check) => inLogicBits.test(logic.items[check][1]));
        ...
        return _.sortBy(areasList, (area) =>
            rawCheckOrder.indexOf(
                area.checks.find((check) => logic.checks[check].type !== 'tr_cube')!,
            ),
        );
...
export const exitsSelector = createSelector(
    [logicSelector, activeVanillaConnectionsSelector, mappedExitsSelector, inLogicBitsSelector], 
